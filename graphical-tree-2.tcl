...
#-- This is very unclean, and is very much in progress
#-- so over look all the little oooo's nothing is clean yet.
#--

    proc s_layer_tree {} {
     
	#----------------------------
	#-------- builds cross hatch in window
	set xsh_line 0
	set ysh_line  [expr $::global(canHeight)/2]
	set xfh_line $::global(canWidth)
	set yfh_line  [expr $::global(canHeight)/2]

	set xsv_line [expr $::global(canWidth)/2]
	set ysv_line 0
	set xfv_line [expr $::global(canWidth)/2]
	set yfv_line $::global(canHeight)

	.c creat line  $xsh_line $ysh_line  $xfh_line  $yfh_line  -width 1\
                                                   -fill black\
                                                   -tag "vert"

	.c creat line  $xsv_line $ysv_line  $xfv_line  $yfv_line  -width 1\
                                                   -fill black\
                                                   -tag "vert"
	#-----------------------------------------------------
           set node 1
		...
		set pi 3.141592
           ...
           # set xf [expr $::global(canWidth)/2]
           # set yf [expr $::global(canHeight)/2]
	     set xf 0
	     set yf 0   
       ...
       .c creat line  $xs $ys  [expr $xp + $xs]  [expr $yp + $ys]\
						 -width 1 \
                                                  -fill blue \
                                                   -tag "vert"
       .c creat line  $xs $ys  [expr (5* $xp) + $xs]  [expr $yp + $ys]\
						 -width 1 \
                                                  -fill green \
                                                   -tag "vert"

              #  .c create text [expr $xf+7] [expr $yf + 7]\
                                        -text "$xp $yp $xf " \
                                        -tag "vert"
